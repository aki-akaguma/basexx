...
#[test]
fn test_base64_g_file_t4_enc() {
    let inp = read_file_data("fixtures/t4.data");
    let oup = read_file_ascii("fixtures/t4.base64.pad.ascii");
    let base64 = Base64G::default();
    assert_eq!(base64.encode(&inp).unwrap(), oup);
}

#[test]
fn test_base64_g_file_t4_dec() {
    let inp = read_file_ascii("fixtures/t4.base64.pad.ascii");
...
fn base64_g_encode_test(#[case] input: &[u8], #[case] output: &str) {
...
#[apply(two_simple_case_1)]
fn base64_g_decode_test(#[case] output: &[u8], #[case] input: &str) {
