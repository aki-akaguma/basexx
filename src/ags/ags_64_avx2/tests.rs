...
#[test]
fn test_ags_binary_to_ascii_64_avx2_1() {
    if !is_x86_feature_detected!("avx2") {
        return;
    }
    let (ags, bin_buf, asc_buf) = gen_data_ags_64();
    let mut buf = bin_buf.clone();
    let r = unsafe { _binary_to_ascii_64_avx2(&ags.binmap, &mut buf) };
    assert!(r.is_ok());
    assert_eq!(buf, asc_buf);
}

#[test]
fn test_ags_binary_to_ascii_64_avx2_2() {
    if !is_x86_feature_detected!("avx2") {
        return;
    }
...
#[test]
fn test_ags_binary_to_ascii_64_avx2_3() {
    if !is_x86_feature_detected!("avx2") {
        return;
    }
