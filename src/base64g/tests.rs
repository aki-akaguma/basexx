...
fn it_works_0() {
    ...
    let r1 = _encode_base64g(&ags, &inp).unwrap();
    assert_eq!(r1, oup);
    let r2 = _decode_base64g(&ags, &r1).unwrap();
    assert_eq!(r2, inp);
}
#[test]
fn it_works_1() {
...
#[test]
fn test_base64g_file_t4_enc() {
    let inp = test_utils::read_t4_data();
    let oup = test_utils::read_t4_base64_pad_ascii();
    let ags = AsciiGraphicSet::with_slice(&_CMAP64);
    let r1 = _encode_base64g(&ags, &inp).unwrap();
    assert_eq!(r1, oup);
}
...
#[test]
fn test_base64g_file_t4_dec() {
    let inp = test_utils::read_t4_base64_pad_ascii();
    let oup = test_utils::read_t4_data();
    let ags = AsciiGraphicSet::with_slice(&_CMAP64);
    let r2 = _decode_base64g(&ags, &inp).unwrap();
    assert_eq!(r2, oup);
}
...
fn base64g_encode_test(#[case] input: &[u8], #[case] output: &str) {
...
#[apply(two_simple_case_1)]
fn base64g_decode_test(#[case] output: &[u8], #[case] input: &str) {
