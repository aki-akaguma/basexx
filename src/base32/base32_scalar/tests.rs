...
fn it_works_0() {
    ...
    assert_eq!(r1, oup);
    let r2 = _decode_base32_scalar(&ags, &r1).unwrap();
    assert_eq!(r2, inp);
}
#[test]
fn it_works_1() {
...
#[test]
fn test_base32_scalar_file_t4_enc() {
...
#[test]
fn test_base32_scalar_file_t4_dec() {
    let inp = test_utils::read_t4_base32_ascii();
    let oup = test_utils::read_t4_data();
    let ags = AsciiGraphicSet::with_slice(&_CMAP32);
    let r2 = _decode_base32_scalar(&ags, &inp).unwrap();
    assert_eq!(r2, oup);
}
...
fn base32_scalar_encode_test(#[case] input: &[u8], #[case] output: &str) {
...
#[apply(two_simple_case_1)]
fn base32_scalar_decode_test(#[case] output: &[u8], #[case] input: &str) {
    let inp = input.to_string();
    let oup = output.to_vec();
    let ags = AsciiGraphicSet::with_slice(&_CMAP32);
    let r1 = _decode_base32_scalar(&ags, &inp).unwrap();
    assert_eq!(r1, oup);
}
