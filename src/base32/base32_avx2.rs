...
pub(crate) unsafe fn _encode_base32_avx2(
    ...
    let oup_idx = _encode_base32_avx2_chunks20(ags, inp, &mut oup[0..])?;
...
pub(crate) unsafe fn _decode_base32_avx2(
    ...
    let mut oup = vec![0u8; oup_sz];
    let oup_idx = _decode_base32_avx2_chunks32(ags, inp, &mut oup[0..])?;
    oup.resize(oup_idx, 0u8);
    Ok(oup)
}
...
pub(crate) unsafe fn _decode_base32_avx2_chunks32(
    ...
    let inp = if inp.len() < 32 {
        ...
        while inp_ptr < end_ptr_limit {
            ...
            std::slice::from_raw_parts_mut(oup_ptr, 5).copy_from_slice(&out32[0..5]);
            std::slice::from_raw_parts_mut(oup_ptr.add(5), 5).copy_from_slice(&out32[5..10]);
            std::slice::from_raw_parts_mut(oup_ptr.add(5 * 2), 5).copy_from_slice(&out32[16..21]);
            std::slice::from_raw_parts_mut(oup_ptr.add(5 * 3), 5).copy_from_slice(&out32[21..26]);
            //
